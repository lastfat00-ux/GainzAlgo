...
## 2024-06-27 - Chronological Navigation in Reverse Layouts
**Learning:** In a `row-reverse` layout where newer data is prepended (index 0), standard Arrow key intuition (Right for 'next') must be mapped to descending indices to maintain a natural chronological flow for keyboard users.
**Action:** Map `ArrowRight` to `index - 1` and `ArrowLeft` to `index + 1` when using `row-reverse` for time-series sparklines.
