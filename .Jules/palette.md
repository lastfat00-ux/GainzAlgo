...
## 2026-09-02 - Disabled Shortcut Feedback & ARIA Keycap Declarations
**Learning:** Triggering tactile keycap depression animations alongside informative screen reader live region announcements when shortcut hotkeys (e.g., `R`, `X`) are pressed while their respective action buttons are disabled or offline prevents perceived interface unresponsiveness and clearly communicates the underlying cause. Furthermore, adding explicit `aria-keyshortcuts` attributes to all shortcut-driven trigger controls ensures assistive technologies declare hotkeys during keyboard navigation.
**Action:** Ensure disabled action shortcut listeners trigger tactile keycap feedback and descriptive live region announcements, and explicitly declare `aria-keyshortcuts` on all hotkey triggers.
