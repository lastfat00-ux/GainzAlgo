...
## 2026-07-04 - Roving Tabindex for Dense Interactive Lists
**Learning:** For high-density interactive tracks like signal history, a Roving Tabindex strategy (where only one item is focusable via Tab, and others are reached via Arrow keys) maintains a clean tab order while ensuring full keyboard navigability.
**Action:** Implement `tabindex="0"` for the primary item and `-1` for others, managing focus transition with Arrow Key listeners.
